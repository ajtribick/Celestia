...
{
public:
    WinAlerter(const std::shared_ptr<SplashWindow>& _splash) : splash(_splash) {};
    ~WinAlerter() {};
...
{
public:
    WinSplashProgressNotifier(const std::shared_ptr<SplashWindow>& _splash) : splash(_splash) {};
...
#endif
{
    ...
    if (dropTarget && CoLockObjectExternal(dropTarget.get(), TRUE, TRUE) != S_OK)
    {
    ...
    {
        AppPreferences prefs;
        if (mainWindow->getCurrentPreferences(prefs))
